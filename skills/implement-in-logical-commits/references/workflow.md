...
- Default commit subjects to omit `#CI`; include it only when the user explicitly asks to include a CI marker on that content commit. Use a separate empty `#CI` trigger commit for ordinary requests to trigger CI or run GitHub Actions after the content commit is published.
...
7. Commit without `#CI` unless the prompt explicitly requested the marker on that content commit, and without amending prior history unless explicitly requested.

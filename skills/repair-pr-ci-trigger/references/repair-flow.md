...
- Require a clean working tree before any checkout, commit, or push.
...
- Verify shell git push access for the exact target before committing. GitHub app access is not enough because the final step is a shell push.
...
- Create only a marker-only empty commit with subject `Trigger CI #CI`.
...
Treat the repair as a no-op only when the pushed tip full commit message already contains a case-insensitive CI marker such as `#CI` or `#ci`. Report the no-op and do not create another empty commit or push.

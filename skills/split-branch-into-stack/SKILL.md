...
7. Commit each coherent slice, preferably using `write-commit-name`; include `#CI` only when the prompt explicitly asks to include a CI marker on that content commit. Use a separate empty `#CI` trigger commit for ordinary requests to trigger CI or run GitHub Actions after the intended branch is published.
